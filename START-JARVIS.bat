...
rem ---------------------------------------------------------------------------
rem Starting Jarvis, and saying what happened when it doesn't start.
rem
...
echo Jarvis launch %DATE% %TIME% (launcher 2026-09-12.1) > "%LOG%"
...
rem order wrong and an Jarvis from this morning answers on the port, this script
...
echo   Getting what Jarvis needs. This takes a minute the first time.
echo.
...
echo   Preparing Jarvis. This happens after an update, not every time.
echo.
...
rem Jarvis rather than a connection error.
...
echo   Close the other Jarvis window, or restart the computer, then try again.
echo.
pause
exit /b 0
...
:wrong_folder
echo   There's no package.json here, so this isn't the Jarvis folder.
echo   This file has to stay next to the rest of Jarvis - if you copied it to
...
:install_failed
echo.
echo   Installing what Jarvis needs didn't work.
echo.
echo   The usual cause is the internet dropping partway through. Run this
echo   again first; if it fails twice, run REBUILD-JARVIS.bat.
goto show_log

:build_failed
echo.
echo   Jarvis didn't build.
echo.
...
rem "Cannot find module" naming a folder Jarvis has never had means files from
rem another project are sitting in src\. tsc compiles everything under there,
rem so they break the build even though nothing in Jarvis imports them. Worth
...
echo   There are files in src\ that aren't part of Jarvis:
echo.
git status --short src 2>nul | findstr /r "^??"
echo.
echo   Those are left over from another project worked on in this folder.
echo   Jarvis compiles everything under src\, so they break the build even
echo   though nothing in Jarvis uses them.
echo.
...
echo.
echo       Run FIX-JARVIS.bat, then REBUILD-JARVIS.bat.
echo.
...
echo   installing: run REBUILD-JARVIS.bat once and it will sort itself out.
goto show_log

:stale
echo   Jarvis is already running, but it's an older version - the update you
echo   pulled hasn't been loaded yet.
echo.
echo   Close the other Jarvis window (the black one that says "Starting"), then
