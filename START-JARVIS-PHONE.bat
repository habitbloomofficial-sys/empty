...
rem Same rule as START-JARVIS.bat: node_modules existing is not the same as it
...
if errorlevel 1 (
  echo   Getting what Jarvis needs. This is a few hundred files, so on a slow
...
rem
rem Jarvis listens on every address on this machine, and always has. What stops
...
echo   right-click START-JARVIS-PHONE.bat, then Run as administrator.
echo.
pause
goto firewall_ok
...
echo.
echo   If it mentions a port already being in use, Jarvis is already running
