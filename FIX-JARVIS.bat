...
echo Jarvis unstick %DATE% %TIME% (launcher 2026-09-12.1) > "%LOG%"
...
echo.
echo   Run CHECK-JARVIS.bat - it will say what IS wrong.
echo.
...
git stash push -m "Jarvis unstick - local package edits" -- package.json package-lock.json >> "%LOG%" 2>&1
...
echo   Jarvis. Jarvis no longer compiles it, so it can no longer break the build -
...
echo   This file has to sit next to the rest of Jarvis.
echo.
echo   Looking in: %CD%
echo.
