...
rem
rem START-JARVIS.bat installs and rebuilds by itself when it needs to, so this is
...
echo Jarvis rebuild %DATE% %TIME% (launcher 2026-09-12.1) > "%LOG%"

echo.
echo   Repairing Jarvis...
echo.
...
echo.
echo   [2/4] Installing everything Jarvis needs. This one takes a few minutes.
...
echo   an old copy.
echo.
echo   Run FIX-JARVIS.bat first. It moves those edits somewhere safe, pulls,
...
:install_failed
echo.
echo   Downloading what Jarvis needs didn't work.
echo.
...
:build_failed
echo.
echo   Everything downloaded fine. Jarvis then failed to BUILD, which has
...
rem so they break the build even though nothing in Jarvis imports them.
git status --short src 2>nul | findstr /r "^??" >nul 2>nul
if errorlevel 1 goto build_failed_plain
echo   I can see why. There are files in src\ that aren't part of Jarvis:
echo.
