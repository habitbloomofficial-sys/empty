...
echo Jarvis check %DATE% %TIME% (launcher 2026-09-12.1) > "%LOG%"
...
if not exist "START-JARVIS.bat" goto no_start_file
for /f "tokens=4" %%v in ('findstr /c:"rem LAUNCHER VERSION" "START-JARVIS.bat"') do set "STAMP_START=%%v"
:no_start_file
if not exist "REBUILD-JARVIS.bat" goto no_rebuild_file
for /f "tokens=4" %%v in ('findstr /c:"rem LAUNCHER VERSION" "REBUILD-JARVIS.bat"') do set "STAMP_REBUILD=%%v"
:no_rebuild_file

echo.
echo   START-JARVIS.bat:    %STAMP_START%
echo   REBUILD-JARVIS.bat:  %STAMP_REBUILD%
echo START-JARVIS.bat: %STAMP_START% >> "%LOG%"
echo REBUILD-JARVIS.bat: %STAMP_REBUILD% >> "%LOG%"
...
echo.
echo       Run FIX-JARVIS.bat  -  it unsticks this without losing anything.
echo.
...
rem belonged to Jarvis, and the build fails naming modules Jarvis has never had.

:local_changes
git status --short src 2>nul | findstr /r "^??" >nul 2>nul
if errorlevel 1 goto no_strays
echo   Files in src\ that are not part of Jarvis:
echo.
git status --short src 2>nul | findstr /r "^??"
git status --short src 2>nul | findstr /r "^??" >> "%LOG%"
echo.
echo   These break the build even though nothing in Jarvis imports them:
...
echo   part of Jarvis. Jarvis no longer compiles anything outside src\, so it
...
:wrong_folder
echo.
echo   There is no package.json here, so this is not the Jarvis folder.
echo   This file has to sit next to the rest of Jarvis. If you copied it to the
