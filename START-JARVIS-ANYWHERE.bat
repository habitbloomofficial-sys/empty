...
title Jarvis - anywhere
cd /d "%~dp0"

echo.
echo   Jarvis, from anywhere
echo   -------------------
echo.
rem LAUNCHER VERSION 2026-09-12.1 - printed so it is obvious which copy is running.
echo   launcher 2026-09-12.1
echo.
echo   This gives Jarvis a web address that works from any phone, on any
...
rem --------------------------------------------------------------------------
rem The passcode is asked for further down, once Jarvis is running - see
...
rem Start Jarvis itself, in its own window, then tunnel to it.
start "Jarvis engine" /min cmd /c "npm run start"

echo   Starting Jarvis and opening the tunnel. This takes a few seconds.
echo.
...
echo.
echo   Jarvis is off the internet again.
pause
exit /b 0
