...
// once Jarvis is installed to the home screen and running without a browser bar.
 ...
 *
 * The installed-app case is the one worth spelling out: a home-screen Jarvis has
...
export function micInstructions(surface: MicSurface, installed: boolean): string {
  if (surface === "ios") {
    return installed
      ? "Open the iPhone Settings app, find Jarvis in the list, and turn Microphone on."
      : "Tap the ᴀA button at the left of the address bar, choose Website Settings, and set Microphone to Allow.";
  }
  if (surface === "android") {
    return installed
      ? "Press and hold the Jarvis icon, tap App info, then Permissions, and allow the Microphone."
